...
- `behavior`: table-of-contents, toolbar, and code-block copy-button behavior plus the local first-run completion marker.
...
The toolbar clipboard action copies the complete raw Markdown source. Each fenced code block additionally carries its own copy button that appears on hover or keyboard focus and copies only that block, without its trailing newline. Turn the per-block buttons off in **Appearance → Reader behavior → Show copy button on code blocks**; they are always excluded from printed output.

Copying uses the browser clipboard API on the local loopback origin and falls back to a hidden text area when that API is unavailable.
