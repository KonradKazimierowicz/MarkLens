param([string]$ApplicationRoot, [string]$ArtifactDirectory)
...
if (-not $edge) { throw 'Microsoft Edge is required for the onboarding test.' }
$node = Get-Command node -ErrorAction SilentlyContinue
if (-not $node) { throw 'Node.js is required for the onboarding test.' }
...
$testRoot = Join-Path $repoRoot ('work\test-onboarding-' + [guid]::NewGuid().ToString('N'))
$dataRoot = Join-Path $testRoot 'data'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

function Invoke-OnboardingRun {
    param([string]$Mode)
    $readyFile = Join-Path $testRoot ("ready-$Mode.txt")
    $server = $null
    try {
        ...
        Assert-True (Test-Path -LiteralPath $readyFile -PathType Leaf) "Onboarding $Mode server did not become ready."
        $url = [IO.File]::ReadAllText($readyFile).Trim()
        $browserArguments = @((Join-Path $PSScriptRoot 'browser-onboarding.mjs'), $edge, $url, $Mode)
        if ($Mode -eq 'first-run' -and -not [string]::IsNullOrWhiteSpace($ArtifactDirectory)) { $browserArguments += [IO.Path]::GetFullPath($ArtifactDirectory) }
        & $node.Source @browserArguments
        if ($LASTEXITCODE -ne 0) { throw "Headless Edge onboarding $Mode assertions failed." }
    }
    finally {
...
try {
    Invoke-OnboardingRun -Mode 'first-run'
    Invoke-OnboardingRun -Mode 'returning'
    Write-Host 'Onboarding browser tests passed.' -ForegroundColor Green
}
finally {
